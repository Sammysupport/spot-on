...
<li><b>GPG-lite via Rosetta:</b> Roseatta is the Conversion pad to tranform plaintext into ciphertext and vice versa. there are not many applications providing such a copy paste conversion tab. Mostly symmetric keys are used. With the GPG introduction in Rosetta it is the first conversion pad for ciphertext, which uses the symmetrical encryption method based on the standard of GPG.</li>
